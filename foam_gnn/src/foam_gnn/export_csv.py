...
## Files (one folder per tracking session)
- `nodes.csv` — one row per (frame, bubble_id).
- `edges.csv` — one row per (frame, bubble_id_i, bubble_id_j).

A **tracking session is a single gap-free (contiguous) run**: bubble IDs are never
matched across an acquisition gap. Most experiments are one run, but `exp1`
(Foam A) contains a 2.5-min internal gap, so it is split into two independent
sessions `exp1_run0` and `exp1_run1` (IDs do NOT carry across the gap). Foam C is
one raft imaged in 5 sessions `exp3`…`exp7`, each tracked independently. The
`session` column identifies the run; never assume an ID in one session is the same
bubble as the same ID in another.

Foam B (exp2) is **excluded**: its higher magnification leaves no background, so
segmentation and the evaporation-edge boundary are not yet reliable. Foams D
(`exp8`) and E (`exp9`) are diagnostic-only foams and are not part of this export.
